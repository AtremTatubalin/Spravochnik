...
public fun SearchScreen(viewModel: GlobalSearchViewModel = GlobalSearchViewModel()) {
    ...
    Column(Modifier.fillMaxSize().padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (state.results.isEmpty()) Text("Введите запрос. Ниже показаны проверочные карточки стандартов и изменений.", color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        Text("Стандарты и изменения", style = MaterialTheme.typography.titleMedium)
        SeedReferenceCatalog.standards.forEach { ReferenceCard(it) }
    }
}
